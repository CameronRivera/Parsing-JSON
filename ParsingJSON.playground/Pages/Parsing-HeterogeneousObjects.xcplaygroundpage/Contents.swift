// Parse Heterogeneous Objects (keys with objects that are mixed types. In this example, post code is a heterogeneous type because it is sometimes a string, and sometimes an integer.) This can lead to decoding errors.
import Foundation
...
enum Postcode: Decodable {
    ...
    // we will also use associated values to capture the value of postcode.
    // e.g. If it is an integer, we will capture the Int value.
    case int(Int)
    case string(String)
    
    // override the init(from decoder:) initalizer
    init(from decoder: Decoder) throws {
        // Handle the two cases of Int or String
        
        // If it is not an Int or a String then we will throw an error.
        // singleValueContainer returns the data contained in the decoder as
        // the type specified.
...
▿ __lldb_expr_5.Person
  - gender: "female"
  ▿ location: __lldb_expr_5.Location
    - city: "Kansas City"
    - country: "United States"
    ▿ postcode: __lldb_expr_5.Postcode.int
      - int: 83696
▿ __lldb_expr_5.Person
  - gender: "female"
  ▿ location: __lldb_expr_5.Location
    - city: "Georgetown"
    - country: "Canada"
    ▿ postcode: __lldb_expr_5.Postcode.string
      - string: "Y5N 0E9"
*/
